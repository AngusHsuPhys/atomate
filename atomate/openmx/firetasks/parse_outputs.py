...
def parse_permittivity_file(calc_dir):
    # Find file like sc_tensor_kgd_*.dat
    pattern = os.path.join(calc_dir, "permittivity_*.dat")
    matches = glob.glob(pattern)
    if not matches:
        return None
    ...
    # Extract k-grid density from filename
    kgd_str = filename.split("permittivity_")[-1].replace(".dat", "")
    try:
        k_grid_density = float(kgd_str)
    except ValueError:
        k_grid_density = None
...
class OpenmxToDb(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        task_doc = drone.assimilate(calc_dir)
        # Optional: parse permittivity file
        try:
            permittivity = parse_permittivity_file(calc_dir)
            if permittivity is not None:
                task_doc["permittivity"] = permittivity
